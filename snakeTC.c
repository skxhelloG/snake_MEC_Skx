...
void die(){
    ...
    if (scores > scoresOld) {
    	fseek(fp, 0, SEEK_SET);
		fprintf(fp, "%s %d", snakeBestHighFen, scores);
		delay(500);
		gotoxy(yCenter-17, xCenter);
    	textcolor(0X04);
    	cprintf("Congratulations! set a new record!");
    }
    getch();
    exit(0);
}
...
void init(){
    ...
    textcolor(0X04);
    cprintf("%s %d", snakeBestHighFen, scoresOld);
    gotoxy(MAXWIDTH/2-4, MAXHEIGHT+1);
    textcolor(0X04);
    cprintf("Fen : ");
    gotoxy(MAXWIDTH-17, MAXHEIGHT+1);
    textcolor(0X04);
    cprintf("direction key:wasd");
    scoresPostion.x = MAXHEIGHT+1;
    scoresPostion.y = MAXWIDTH/2+2;
    gotoxy(scoresPostion.y, scoresPostion.x);
    textcolor(0X02);
    cprintf("%d", scores);
}
...
int main(){
    ...
    fscanf(fp, "%s%d", snakeBestHighFen, &scoresOld);
    init();
