...
#define MAXWIDTH    (40)//最大列
#define MAXHEIGHT   (20)//最大行
...
void move(char direction){
    ...
    if ( globalMap[newHeaderX][newHeaderY].type == charBorder.type || 
        ...
        textcolor(0X02);
        scores += fen;
        cprintf("%d", scores);//对分数的修改
    } else  {
        oldTailX = snakeMap[tailIndex].x;
        oldTailY = snakeMap[tailIndex].y;//移动前蛇尾坐标
...
void init(){
    ...
    textcolor(0X04);
    for(x = 7; x <= MAXHEIGHT-2; x++){  //对障碍的必要处理部分
    ...
    cprintf("      Author : skx");
    gotoxy(MAXWIDTH+2, MAXHEIGHT);
    cprintf("CreationTime : 2019/07/28");
    scoresPostion.x = MAXHEIGHT-2;
    scoresPostion.y = MAXWIDTH+17;
    gotoxy(scoresPostion.y, scoresPostion.x);
    textcolor(0X02);
    cprintf("%d", scores);
}
